...
from bot_email import send_bot_email
...
class ServiceBuilder:
  def __init__(self, config=None):
    ...
    # scripts:
    self.local_start_script = config["local_start_script"]
    # data:
  ...
  def start_local_service(self):
    wd = dirname(self.local_start_script)
    self._check_call(["sh", self.local_start_script], wd=wd)

  def test_local_service_and_release(self):
    local_tester_config = copy(bs_service_tester.tester_config)
    local_tester_config["service_host"] = "localhost"
    service_tester = ServiceTester(local_tester_config)
  ...
  def archive_war(self):
    ...
    files = os.listdir(self.war_archive_dir)
    ...
    if len(archives) > self.max_war_archives:
      ...
      os.remove(join(self.war_archive_dir, archives[0]))
...
if __name__ == "__main__":
  ...
  try:
    ...
    send_bot_email("Dev build failed.", str(e))
